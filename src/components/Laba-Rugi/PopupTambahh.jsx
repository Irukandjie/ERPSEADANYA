...
const PopupTambah = ({ isOpen, onClose, onSubmit }) => {
  const [formData, setFormData] = useState({ nama: '', nilai: '', kategori: 'operasional' });
  ...
  const handleSubmit = (e) => {
    ...
    setFormData({ nama: '', nilai: '', kategori: 'operasional' });
    onClose();
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative z-10 bg-white dark:bg-slate-800 w-full max-w-[420px] rounded-[1.75rem] shadow-2xl border border-slate-100 dark:border-slate-700 overflow-hidden transform scale-100 transition-all">
        <div className="p-5 md:p-6 border-b border-slate-100 dark:border-slate-700 bg-emerald-50/50 dark:bg-emerald-900/20 flex justify-between items-center">
          <h3 className="font-black text-xl text-emerald-900 dark:text-emerald-400">Tambah Data Laba Rugi</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <input type="text" placeholder="Cth: Tagihan Pasien Umum" required value={formData.nama} onChange={(e) => setFormData({...formData, nama: e.target.value})} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 outline-none focus:border-emerald-500" />
          </div>
          ...
          <div>
            ...
            <select value={formData.kategori} onChange={(e) => setFormData({...formData, kategori: e.target.value})} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 outline-none focus:border-emerald-500">
          ...
          <div>
            ...
            <input type="number" placeholder="0" required min="1" value={formData.nilai} onChange={(e) => setFormData({...formData, nilai: e.target.value})} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 outline-none focus:border-emerald-500" />
          </div>

          <div className="pt-3">
            <button type="submit" className="w-full bg-emerald-500 hover:bg-emerald-400 text-white font-extrabold py-3.5 rounded-xl transition-all shadow-md shadow-emerald-500/20">
              Simpan Data Baru
            </button>
          </div>
        </form>
