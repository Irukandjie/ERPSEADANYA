...
const PopupHapus = ({ isOpen, onClose, onConfirm, dataHapus }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative z-10 bg-white dark:bg-slate-800 w-full max-w-[380px] rounded-[1.75rem] shadow-2xl border border-slate-100 dark:border-slate-700 overflow-hidden transform scale-100 transition-all text-center p-7">
        ...
        <h3 className="text-xl font-black text-slate-800 dark:text-white mb-2">Hapus Data Ini?</h3>
        <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-6 leading-relaxed">
          Apakah Anda yakin ingin menghapus data <strong className="text-slate-700 dark:text-slate-300">"{dataHapus.nama}"</strong>? Tindakan ini tidak dapat dibatalkan.
        </p>
        ...
        <div className="flex gap-3">
          ...
          <button 
            onClick={() => {
              onConfirm(dataHapus.id, dataHapus.kategori);
              onClose();
            }} 
            className="flex-1 bg-rose-500 hover:bg-rose-600 text-white font-bold py-3 rounded-xl transition-all shadow-md shadow-rose-500/20"
          >
            Ya, Hapus
          </button>
        </div>
