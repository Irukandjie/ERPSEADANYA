...
const ChartGrafik = ({ transactions }) => {
  ...
  // UPDATE: Fungsi buat nyingkat tanggal di Sumbu X (Cth: "2026-09-01" jadi "01 Sep")
  const formatDateXAxis = (dateStr) => {
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-md p-4 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-[0_20px_50px_-10px_rgba(0,0,0,0.15)] min-w-[200px] transition-colors">
          <p className="text-xs font-black text-slate-400 dark:text-slate-500 mb-3 border-b border-slate-100 dark:border-slate-700/50 pb-2 uppercase tracking-widest">{formatDateXAxis(label)}</p>
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-[1.25rem] shadow-sm border border-slate-100 dark:border-slate-700/50 p-5 md:p-8 mb-8 relative overflow-hidden transition-colors duration-300">
      <div className="flex items-center gap-3 md:gap-4 mb-6 md:mb-8 relative z-10">
        <div className="bg-gradient-to-br from-indigo-500 to-indigo-600 w-10 h-10 md:w-12 md:h-12 rounded-[1rem] text-white flex items-center justify-center shadow-lg shadow-indigo-200/50 dark:shadow-none transition-colors flex-shrink-0">
          <svg className="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
          </svg>
        </div>
        <div>
          <h2 className="font-extrabold text-slate-800 dark:text-white text-xl md:text-2xl tracking-tight transition-colors">Grafik Arus Kas</h2>
          <p className="text-[10px] md:text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-0.5 md:mt-1 transition-colors">Statistik Pemasukan vs Pengeluaran</p>
        </div>
      </div>

      <div className="w-full h-[300px] md:h-[340px] relative z-10 -ml-4 md:-ml-0">
        {chartData.length > 0 ? (
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 10 }} barGap={6}>
              ...
              <XAxis 
                ...
                tickFormatter={formatDateXAxis} // Terapin formatter di sini
                dy={15}
              />
              ...
              <Legend 
                iconType="circle" 
                wrapperStyle={{ paddingTop: '25px', fontSize: '12px', fontWeight: '800', color: '#64748b' }} 
              />
              
              <Bar dataKey="Pemasukan" fill="url(#colorPemasukan)" radius={[6, 6, 0, 0]} maxBarSize={40} />
              <Bar dataKey="Pengeluaran" fill="url(#colorPengeluaran)" radius={[6, 6, 0, 0]} maxBarSize={40} />
            </BarChart>
          </ResponsiveContainer>
        ) : (
          <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 transition-colors">
            ...
            <p className="text-sm md:text-base font-bold text-slate-500 dark:text-slate-400">Belum ada data transaksi</p>
          </div>
        )}
      </div>
    </div>
