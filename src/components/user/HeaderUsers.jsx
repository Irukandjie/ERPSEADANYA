...
const HeaderUsers = ({ totalUsers, activeUsers, onOpenModal }) => {
  return (
    <>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-5 mb-7 transition-colors">
        <div className="flex items-center gap-3 sm:gap-4 flex-1">
          ...
          <div className="min-w-0 pr-2">
            <h2 className="font-extrabold text-slate-800 dark:text-white text-xl sm:text-2xl tracking-tight transition-colors truncate">Manajemen Users</h2>
            <p className="text-[11px] sm:text-sm font-medium text-slate-400 dark:text-slate-500 mt-0.5 transition-colors truncate">Kelola akses dan daftar staf pengguna</p>
          </div>
        </div>
      ...
      <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-[1.25rem] p-6 md:p-8 text-white shadow-lg shadow-blue-500/20 dark:shadow-none relative overflow-hidden mb-7 transition-colors">
        <div className="relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-5">
          <div>
            <h6 className="text-[10px] md:text-xs font-extrabold uppercase tracking-widest text-blue-100 mb-2">Total Pengguna Sistem</h6>
            <h3 className="text-3xl md:text-5xl font-black tracking-tight">{totalUsers} <span className="text-xl md:text-2xl font-bold opacity-80">Akun</span></h3>
          </div>
          <div className="flex items-center gap-2.5 bg-white/20 dark:bg-slate-900/30 backdrop-blur-md px-4 py-2.5 md:px-5 md:py-3 rounded-xl border border-white/20 dark:border-slate-700/50 transition-colors w-full sm:w-auto justify-between sm:justify-start">
            <span className="text-[11px] md:text-xs font-bold text-blue-50">Sedang Aktif:</span>
            <span className="text-sm md:text-base font-black text-white bg-blue-800/40 dark:bg-blue-900/60 px-2 md:px-2.5 py-0.5 rounded-md transition-colors">
              {activeUsers} User
            </span>
          </div>
        </div>
