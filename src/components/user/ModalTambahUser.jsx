...
const ModalTambahUser = ({ isOpen, onClose, onSubmit }) => {
  const [formData, setFormData] = useState({
    nama: '', email: '', role: 'Staff Keuangan', status: 'Aktif'
  });
  ...
  useEffect(() => {
    if (isOpen) {
      setFormData({ nama: '', email: '', role: 'Staff Keuangan', status: 'Aktif' });
    }
  }, [isOpen]);
  ...
  return createPortal(
    <>
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
        ...
        <div className="relative z-10 bg-white dark:bg-slate-800 w-full max-w-[420px] rounded-[1.75rem] shadow-[0_20px_60px_-10px_rgba(0,0,0,0.15)] border border-slate-100 dark:border-slate-700 animate-modal-pop overflow-hidden transition-colors">
          
          <div className="p-6 border-b border-slate-100 dark:border-slate-700/80 bg-blue-50/50 dark:bg-slate-800/90 flex justify-between items-center transition-colors">
            <div className="flex items-center gap-3">
               ...
               <h3 className="font-black text-xl text-blue-900 dark:text-white tracking-tight transition-colors">Tambah User</h3>
            </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            <div>
              <label className="block text-xs font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5 ml-0.5">Nama Lengkap</label>
              <input type="text" name="nama" placeholder="Cth: Alden Waruwu" required value={formData.nama} onChange={handleChange} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 outline-none focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition-all placeholder-slate-400 dark:placeholder-slate-500" />
            </div>
            
            <div>
              <label className="block text-xs font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5 ml-0.5">Email Akses</label>
              <input type="email" name="email" placeholder="Cth: alden@erp.com" required value={formData.email} onChange={handleChange} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 outline-none focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition-all placeholder-slate-400 dark:placeholder-slate-500" />
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5 ml-0.5">Hak Akses</label>
              ...
              <div>
                <label className="block text-xs font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5 ml-0.5">Status Akun</label>
            ...
            <div className="pt-3">
              <button type="submit" className="group relative w-full bg-blue-600 hover:bg-blue-500 dark:bg-blue-600 dark:hover:bg-blue-500 text-white text-[15px] font-extrabold py-3.5 rounded-xl transition-all duration-300 active:scale-[0.98] border border-transparent hover:shadow-[0_8px_25px_0_rgba(37,99,235,0.4)] dark:hover:shadow-none overflow-hidden">
                ...
                <span className="relative z-10">Undang User Baru</span>
              </button>
            </div>
          </form>
