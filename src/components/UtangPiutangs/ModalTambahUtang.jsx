    ...
    <>
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
        ...
        <div className="relative z-10 bg-white/95 dark:bg-slate-800 backdrop-blur-3xl w-full max-w-[460px] rounded-[1.75rem] shadow-[0_20px_60px_-10px_rgba(0,0,0,0.15)] border border-white/80 dark:border-slate-700 animate-modal-pop overflow-hidden transition-colors">
          ...
          <div className="p-6 border-b border-slate-100/50 dark:border-slate-700/80 bg-teal-50/50 dark:bg-slate-800/90 flex justify-between items-center transition-colors">
            <div className="flex items-center gap-3">
               <div className="w-10 h-10 rounded-[1rem] bg-teal-500 text-white flex items-center justify-center shadow-lg shadow-teal-200/60 dark:shadow-none">
               ...
               <div>
                  <h3 className="font-black text-xl text-teal-900 dark:text-white tracking-tight leading-tight transition-colors">Catat Tagihan</h3>
                  <p className="text-[10px] font-bold text-teal-600/70 dark:text-teal-400 mt-0.5 transition-colors">Form Hutang Piutang RS</p>
               </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            <div>
              <label className="block text-xs font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5 ml-0.5">Nama Vendor / Keterangan</label>
              <input type="text" name="namaPihak" placeholder="Cth: Klaim BPJS Agt / PT Kimia Farma" required value={formData.namaPihak} onChange={handleChange} className="w-full px-4 py-3 bg-white/50 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 outline-none focus:ring-4 focus:ring-teal-500/20 focus:border-teal-500 transition-all placeholder-slate-400 dark:placeholder-slate-500" />
            </div>
            
            <div>
              <label className="block text-xs font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5 ml-0.5">Kategori Transaksi</label>
              <select name="kategori" value={formData.kategori} onChange={handleChange} className="w-full px-4 py-3 bg-white/50 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 outline-none focus:ring-4 focus:ring-teal-500/20 focus:border-teal-500 transition-all">
                <optgroup label="PIUTANG (Uang RS yang Masih Nyangkut)" className="dark:bg-slate-800">
                ...
                <optgroup label="UTANG (Kewajiban RS ke Pihak Lain)" className="dark:bg-slate-800">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5 ml-0.5">Jatuh Tempo</label>
                <input type="date" name="jatuhTempo" required value={formData.jatuhTempo} onChange={handleChange} className="w-full px-4 py-3 bg-white/50 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 outline-none focus:ring-4 focus:ring-teal-500/20 focus:border-teal-500 transition-all" style={{ colorScheme: 'auto' }} />
              </div>
              <div>
                <label className="block text-xs font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5 ml-0.5">Status</label>
            ...
            <div>
              <label className="block text-xs font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5 ml-0.5">Nominal (Rp)</label>
              <input type="number" name="jumlah" placeholder="0" required min="1" value={formData.jumlah} onChange={handleChange} className="w-full px-4 py-3 bg-white/50 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 outline-none focus:ring-4 focus:ring-teal-500/20 focus:border-teal-500 transition-all" />
            </div>

            <div className="pt-3">
              <button type="submit" className="group relative w-full bg-teal-500 hover:bg-teal-400 dark:bg-teal-600 dark:hover:bg-teal-500 text-white text-[15px] font-extrabold py-3.5 rounded-xl transition-all duration-300 active:scale-[0.98] border border-transparent hover:shadow-[0_8px_25px_0_rgba(20,184,166,0.4)] dark:hover:shadow-none overflow-hidden">
