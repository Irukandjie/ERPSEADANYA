...
const ModalKasKeluar = ({ isOpen, onClose, onSubmit }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setFormData({ tanggal: today, deskripsi: '', tipe: 'Pengeluaran Kas - Tunai', jumlah: '' });
    }
  }, [isOpen]);
    ...
    <>
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
        ...
        <div className="relative z-10 bg-white dark:bg-slate-800 w-full max-w-5xl rounded-[1.75rem] shadow-[0_20px_60px_-10px_rgba(0,0,0,0.15)] border border-slate-100 dark:border-slate-700 animate-modal-pop overflow-hidden transition-colors">
          ...
          <div className="p-5 md:p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-rose-50/50 dark:bg-slate-800/80 transition-colors">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-[1rem] bg-rose-500 text-white flex items-center justify-center shadow-lg shadow-rose-200/60 dark:shadow-none">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4" transform="rotate(45 12 12)" /></svg>
              </div>
              <h2 className="font-extrabold text-xl tracking-tight text-slate-800 dark:text-white">Catat Kas Keluar</h2>
            </div>
          ...
          <div className="p-6 md:p-8 bg-white dark:bg-slate-800 transition-colors">
            <form onSubmit={handleSubmit} className="flex flex-col lg:flex-row items-end gap-5">
              <div className="w-full lg:w-[15%]">
                ...
                <input type="date" name="tanggal" required value={formData.tanggal} onChange={(e) => setFormData({...formData, tanggal: e.target.value})} className="w-full px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none transition-all text-sm font-medium text-slate-700 dark:text-slate-200 focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500" style={{ colorScheme: 'auto' }} />
              </div>
              
              <div className="w-full lg:w-[35%]">
                <label className="block text-sm font-bold text-slate-500 dark:text-slate-400 mb-1.5 ml-0.5">Keterangan / Deskripsi</label>
                <input type="text" name="deskripsi" placeholder="Cth: Bayar Listrik / Operasional" required value={formData.deskripsi} onChange={(e) => setFormData({...formData, deskripsi: e.target.value})} className="w-full px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none transition-all text-sm font-medium text-slate-700 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500" />
              </div>

              <div className="w-full lg:w-[20%]">
                <label className="block text-sm font-bold text-slate-500 dark:text-slate-400 mb-1.5 ml-0.5">Tipe (Sesuai Kolom)</label>
                <select name="tipe" required value={formData.tipe} onChange={(e) => setFormData({...formData, tipe: e.target.value})} className="w-full px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none transition-all text-sm font-bold text-slate-700 dark:text-slate-200 focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500">
                  <optgroup label="BUKU KAS" className="dark:bg-slate-800">
                    <option value="Pengeluaran Kas - Tunai">Kas Tunai</option>
                    <option value="Pengeluaran Kas - Obat">Kas Obat</option>
                  </optgroup>
                  <optgroup label="BUKU BANK (KREDIT/KELUAR)" className="dark:bg-slate-800">
                    <option value="Panin - Kredit">Panin (-)</option>
                    <option value="BCA - Kredit">BCA (-)</option>
                    <option value="Mandiri - Kredit">Mandiri (-)</option>
                    <option value="BSI - Kredit">BSI (-)</option>
                    <option value="BNI - Kredit">BNI (-)</option>
                    <option value="BRI - Kredit">BRI (-)</option>
                  </optgroup>
                </select>
              </div>
              ...
              <div className="w-full lg:w-[15%]">
                ...
                <input type="number" name="jumlah" placeholder="0" required min="1" value={formData.jumlah} onChange={(e) => setFormData({...formData, jumlah: e.target.value})} className="w-full px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none transition-all text-sm font-medium text-slate-700 dark:text-slate-200 focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500" />
              </div>

              <div className="w-full lg:w-[15%]">
                <button type="submit" className="group relative w-full bg-rose-500 hover:bg-rose-400 dark:bg-rose-600 dark:hover:bg-rose-500 text-white text-[15px] font-bold py-3.5 rounded-xl transition-all duration-300 active:scale-[0.98] border border-transparent hover:shadow-[0_8px_25px_0_rgba(244,63,94,0.4)] overflow-hidden">
